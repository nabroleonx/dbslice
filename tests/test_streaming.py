...
def test_streaming_output_not_clobbered_by_output_handler(sample_schema, tmp_path):
    """
    Regression test: when ExtractionResult has was_streamed=True, the cli
    output handlers must not regenerate SQL from result.tables and write it
    to out_file. In streaming mode, tables is intentionally empty (data
    already lives in the file); regenerating and writing would clobber the
    streamed content with an empty BEGIN/COMMIT shell.

    See https://github.com/nabroleonx/dbslice/issues/8.
    """
    import rich.console
    from dbslice.cli import _generate_and_output_sql
    from dbslice.core.engine import ExtractionResult

    # Simulate the file the streaming engine already wrote.
    streamed_content = (
        "-- Generated by dbslice (streaming mode)\n"
        "BEGIN;\n"
    ...
    out_file.write_text(streamed_content)
    ...
    returned = _generate_and_output_sql(
        result=result,
        schema=sample_schema,
    ...
    # The file must still contain the streamed content — not be replaced
    # with an empty structured shell.
    assert out_file.read_text() == streamed_content
